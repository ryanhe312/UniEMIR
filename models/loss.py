...
def mix_loss(output, target):
    return F.mse_loss(output, target) + F.l1_loss(output, target)
...
# def lpips_mix_loss(output, target):
    # # channel-wise LPIPS
    # lpips_loss = vgg_model(output.view(-1, 1, 256, 256), target.view(-1, 1, 256, 256)).mean()
    # return 0.01 * lpips_loss + F.mse_loss(output, target) + F.l1_loss(output, target)    
