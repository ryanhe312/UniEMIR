...
from models.UniEMIR.enlcn import ENLCN
...
class RSTB(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False,
                 img_size=224, patch_size=4, resi_connection='1conv', merge=False, split=False):
        ...
        self.pruned = False
        self.conv_merge = nn.Conv2d(int(dim*4), dim, 3, 1, 1)
        self.conv_split = nn.Conv2d(int(dim/4), dim, 3, 1, 1)
    ...
    def forward(self, x, x_size: Tuple[int,int]):
        ...
        if self.merge: 
            x = self.patch_unembed(x, (x_size[0] * 2, x_size[1] * 2))
        ...
        if self.split:
            y2 = self.patch_unembed(y2, x_size)
            y2 = F.pixel_shuffle(y2, 2)
            y2 = self.conv_split(y2)
            y2 = self.patch_embed(y2)
